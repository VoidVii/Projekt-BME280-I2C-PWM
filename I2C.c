...
*@return		Returns specific Error-Code if an error accures 
*/
...
*@param module 		Used I2C - module (0-3)
*@param devSlave	Slave device address (7-bit)
*@param reg 			Slave device register to read from
*@param buffer		Buffer to store read data bytes 
*@param len				How many registers to read 
*
*@return	Returns the data from the slave data-registers or errors
*/
int32_t I2C_ReadBurst(uint8_t module, uint8_t devSlave, uint8_t reg, uint8_t* buffer, uint8_t length)
{
	...
	*msa = ((devSlave << 1U) | 1U); /**< 0. Bit = R/S bit ==> Write -> 1. */
	...
	for(uint8_t volatile i = 0U; i < length; i++)
	{
		...
		{
			/* Master writes 0x7 to the I2CMCS register to initiate and end a transfer */
			*mcs = START_RUN_STOP;
		}
		else if(i == 0U && length > 1U)
		{
			/* Master writes 0xB to the I2CMCS register to initiate a transfer */
			*mcs = START_RUN_ACK;
		}
		else if(i == (length - 1U))
		{
			/* Master writes 0x5 to the I2CMCS register to end a transfer */
			*mcs = RUN_STOP;
		}
		else
		{
			/* Master writes 0x9 to the I2CMCS register to initiate next transfer */
			*mcs = RUN_ACK;
		}

		/* Wait until I2C controller is not busy (BUSY bit = 0) and check for error */
		/* If error accured return status */
		status = I2C_Bus_Busy_Wait(mcs);
